...
export function initScheduleModule(app) {
    app.renderScheduleTable = function(sched, editable = false, grade = null, cls = null) {
        ...
        let html = `<div class="${window.innerWidth <= 768 ? 'sticky-first-col' : ''}"><table class="main-table"><thead><tr><th>اليوم</th><th>1</th><th>2</th><th>3</th><th>4</th><th>5</th><th>6</th><th>7</th></tr></thead><tbody>`;
    ...
    app.renderStudentSchedule = function() {
        ...
        app.addFullscreenButton('#tab-schedule .card');
    ...
    app.editTeacherCell = async function(teacherId, day, period) {
        ...
        if (n !== null) {
            if (!db.teacherSchedules[teacherId]) db.teacherSchedules[teacherId] = {};
            if (!db.teacherSchedules[teacherId][day]) db.teacherSchedules[teacherId][day] = {};
    ...
    app.renderStudentData = function() {
        ...
        if (count === 0) html += `<tr><td colspan="7" style="color:#888;">لا توجد علامات</td></tr>`;
        html += `</tbody></table>`; // تم إغلاق الجدول هنا بشكل صحيح

        if (count > 0) {
            const percentage = ((totalSum / (count * 100)) * 100).toFixed(1);
            html += `<div style="margin-top:15px; background:#eef2f5; padding:10px; border-radius:12px; text-align:center;">📈 المعدل المئوي: ${percentage}% <button onclick="app.downloadStudentOwnReport()" style="background:var(--primary-dark); color:white; border:none; padding:6px 12px; border-radius:20px;">📥 تحميل الشهادة PDF</button></div>`;
        }
        div.innerHTML = html;
    };
