// admin.js
import { db, gradesConfig, subjectsList, updateClassTeacher, updateTeacherAssignment, addNotification } from "./db.js";
...
export function initAdminModule(app) {
    ...
    app.renderAssignmentPanel = function(teacherId) {
        ...
        if (!container) return;
        const assignments = db.teacherAssignments[teacherId] ? [db.teacherAssignments[teacherId]] : [];
    ...
    app.saveTeacherSubject = async function(teacherId) {
        ...
        if (error) return showToast("فشل الحفظ", "error");
        db.teacherAssignments[teacherId] = { subject, classes };
    ...
    app.removeTeacherSubject = async function(teacherId, index) {
        ...
        delete db.teacherAssignments[teacherId];
        app.renderAssignmentPanel(teacherId);
    };
