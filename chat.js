...
import { updateGroup, saveDirectMessages, addNotification, addContactToSupabase, removeContactFromSupabase } from "./db.js";
import { supabase } from "./supabase-config.js";

// دالة مساعدة لإرجاع الصورة المناسبة لجهة اتصال (تطابق ما في core.js)
function getUserAvatar(user) {
    if (!user) return 'avatar-student-789.png'; // افتراضي
    if (db.pfp[user.s]) return db.pfp[user.s];
    switch (user.r) {
        case 'admin':
            return 'avatar-admin.png';
        case 'teacher':
            return 'avatar-teacher.png';
        case 'student':
            if (['خامس', 'سادس'].includes(user.grade)) {
                return 'avatar-student-56.png';
            } else if (['سابع', 'ثامن', 'تاسع'].includes(user.grade)) {
...
export function initChatModule(app) {
    ...
    app.replyToMessage = function(index) {
        const msgs = app.getCurrentChatMessages();
        app.replyingTo = msgs[index];
        const preview = document.getElementById('reply-preview');
        if (preview) {
            preview.style.display = 'block';
            preview.innerHTML = `
                <div style="background:var(--bg-card); padding:8px; border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
                    <span>↩ الرد على: <strong>${app.replyingTo.senderName}</strong> "${app.replyingTo.text?.substring(0,40)}..."</span>
                    <button onclick="app.cancelReply()" style="background:none; border:none; color:red; font-size:1.2rem;">✕</button>
                </div>`;
        }
    ...
    app.sendMessage = async function() {
        ...
        const msg = {
            ...
            deletedFor: [],
            readBy: [app.user.s],
            replyTo: app.replyingTo ? {
                senderName: app.replyingTo.senderName,
                text: app.replyingTo.text?.substring(0,80)
            } : null
        };
        ...
        app.replyingTo = null;
        const replyDiv = document.getElementById('reply-preview');
        if (replyDiv) { replyDiv.style.display = 'none'; replyDiv.innerHTML = ''; }
    ...
    app.renderMessages = function() {
        ...
        msgs.forEach((m, idx) => {
            ...
            let content = '';
            if (m.replyTo) {
                content += `<div class="reply-to" style="font-size:0.8rem; opacity:0.8; border-right:3px solid var(--primary-light); padding-right:6px; margin-bottom:4px;">↩ <strong>${m.replyTo.senderName}:</strong> ${linkify(m.replyTo.text)}</div>`;
            }
            ...
            let readMark = '';
            if (app.activeChat.t === 'user' && m.senderId === app.user.s) {
                const otherId = app.activeChat.id;
                const read = m.readBy?.includes(otherId);
                readMark = `<span style="color:${read?'#4CAF50':'#999'}; margin-right:5px; font-size:0.8rem;">${read ? '✓✓' : '✓'}</span>`;
            }
            let timeHtml = `<span class="time">${m.time}${readMark}</span>`;
            ...
            html += `<div class="${className}" data-msg-index="${idx}">
                        ...
                        <div class="bubble">${content}${timeHtml}${reactions}</div>
                    </div>`;
    ...
    function sendTypingSignal() {
        if (!app.activeChat || !app.user) return;
        supabase.channel('typing').send({
            type: 'broadcast',
            event: 'typing',
            payload: { chatId: app.activeChat.id, userId: app.user.s, name: app.user.n, time: Date.now() }
        });
    }
        ...
        chatInput.addEventListener('input', () => {
            ...
            typingTimer = setTimeout(() => {
                supabase.channel('typing').send({
                    type: 'broadcast',
                    event: 'stop_typing',
                    payload: { chatId: app.activeChat?.id, userId: app.user?.s }
                });
            }, 2000);
        });
    }

    window.addEventListener('typing-event', (e) => {
        if (e.detail.userId !== app.user?.s && app.activeChat?.id === e.detail.chatId) {
            const indicator = document.getElementById('typing-indicator');
            if (indicator) {
                indicator.textContent = `${e.detail.name} يكتب...`;
    ...
    window.addEventListener('stop-typing-event', (e) => {
        if (e.detail.userId !== app.user?.s && app.activeChat?.id === e.detail.chatId) {
            const indicator = document.getElementById('typing-indicator');
    ...
    app.showMessageContext = function(event, index) {
        ...
        const reply = createMenuItem('↩️ رد', () => { app.replyToMessage(index); menu.remove(); });
        ...
        menu.append(copy, reply, delMe, delAll, separator, emojiTitle, emojiContainer);
    ...
    app.openChat = function(type, id) {
        ...
        if (type === 'user') {
            const cid = [app.user.s, id].sort().join('_');
            const msgs = db.directMessages[cid] || [];
            let updated = false;
            msgs.forEach(m => {
                if (!m.readBy) m.readBy = [];
                if (m.senderId !== app.user.s && !m.readBy.includes(app.user.s)) {
                    m.readBy.push(app.user.s);
                    updated = true;
                }
            });
            if (updated) {
                db.directMessages[cid] = msgs;
                saveDirectMessages(cid, msgs);
            }
        }
    ...
    app.renderChatHeader = function() {
        ...
        header.innerHTML = `<button class="back-btn" onclick="app.goBackToChatList()">←</button> <span>محادثة مع <b>${name}</b></span> ${leaveBtn} <span id="typing-indicator" style="color:#FFD54F; font-size:0.85rem; display:none;"></span>`;
    };
    ...
    app.forceLeaveGroup = async function(groupId) {
        ...
        group.messages.push({ senderId:"system", senderName:"النظام", time:new Date().toLocaleTimeString(), text:`📢 خرج الأستاذ ${app.user.n} من المجموعة`, images:[], reactions:[], deletedFor:[], readBy:[] });
    ...
    // الوظائف المعدلة لمزامنة جهات الاتصال مع Supabase
    app.deleteContact = async function(serial) {
        try {
            await removeContactFromSupabase(app.user.s, serial);
            if (db.contacts[app.user.s]) {
                db.contacts[app.user.s] = db.contacts[app.user.s].filter(s => s !== serial);
            }
            app.syncLocalStorage();
            app.renderChatList();
            showToast("تم حذف جهة الاتصال", "success");
        } catch (err) {
            showToast("فشل حذف جهة الاتصال", "error");
        }
    };

    app.addContact = async function() {
        ...
        try {
            await addContactToSupabase(app.user.s, serial);
            ...
            showToast("فشل إضافة جهة الاتصال، تحقق من الاتصال", "error");
        }
    };
}