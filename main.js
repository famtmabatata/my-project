// main.js - النسخة النهائية Supabase (جزء 1/3)
...
const app = {
    ...
    async upsertGrade(student_serial, subject, m1, m2, m3, fin) {
        await supabase.from('grades').upsert({ student_serial, subject, m1, m2, m3, fin });
    },
    ...
    async doLogin() {
        ...
        if (hashed.startsWith('sha256:')) {
            ...
            if (!ok) { this.showToast("البيانات خاطئة!", "error"); return; }
        } else {
            if (p !== hashed) { this.showToast("البيانات خاطئة!", "error"); return; }
    ...
    renderGroupManagement() {
        ...
        // تصحيح: استخدام function() بدلاً من () => للوصول إلى this.value الصحيح
    ...
    async createGroupFromMgmt() {
        ...
        // إدخال بدون id، ونجعل Supabase يولد المفتاح
    ...
    renderMessages() {
        ...
        msgs.forEach((m, idx) => {
            ...
            html += `<div class="message ${m.senderId === this.user.s ? 'sent' : 'received'}" data-msg-index="${idx}">
                        ${this.activeChat.t === 'group' && m.senderId !== this.user.s ? `<small>${m.senderName}</small>` : ''}
